...
jsPsych.plugins["custom-form"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      email_valid: {
        ...
        description: "Text if email not valid"
      },
      number_valid: {
        type: jsPsych.plugins.parameterType.STRING,
        default: 'Number only',
        description: "Text if number not valid"
      },
      select_start: {
        type: jsPsych.plugins.parameterType.STRING,
      ...
      button_change: {
        ...
        default: null,
        description: "For a page that asks participants to choose a language to continue. This allows the text of the continue button to be updated based on the language selected"
      }
    }
  };
  ...
  plugin.trial = function(display_element, trial) {
    ...
    */
    ...
    // css += '.inline-contents > div:not(.reminder .problem) {display: inline-block}';
    css += '.flex {display: flex;}';
    css += '.response {margin-left: 2vw;}';
    css += '.specify {display: inline-block; color: black}';
    css += '.highlight {background-color: #daedf2}';
    css += '.followup {display: inline-block; padding-left: 20px}';
    css += '.indent {margin-left: 24px;}';
    css += '.multiple-container {padding: 0px 5px;}';
    ...
    css += '.start {padding-top: 18px;}';
    css += '.end {padding-bottom: 18px;}';
    ...
    trial.questions.forEach(function(question, q_index){
      ...
      if(question.group){
        if(question.group > group || question.group==0){
    ...
    function text(question_data, q_index){
      ...
      if(question_data.numeric){
        html_string += '<div id="'+question_id+'-reminder" class="reminder">'+trial.number_valid+'</div>';
      }
      if(question_data.email){
        html_string += '<div id="'+question_id+'-reminder" class="reminder">'+trial.email_valid+'</div>';
      }
      html_string += '</div>';
    ...
    function checkbox(question_data, q_index){
      ...
      var html_string = '<div id="question-container-'+question_id+'" class="'+container_class_string+'">';
      html_string += '<div class="prompt">'+prompt+'</div>';
      html_string += '<div id="'+question_id+'" class="custom-form-checkbox flex" style="width:100%; flex-wrap: wrap;">';
      var option_string = '';
      var width_string;
      ...
      options.forEach(function(option, i){
        ...
        if(question_data.disable){
          if(question_data.disable.indexOf(value_string)!=-1){
            if(!disable[value_string]){
              disable[value_string] = [];
            }
            disable[value_string].push(question_id);
          }
        }
    ...
    function select(question_data, q_index){
      ...
      if(question_data.specify){
        html_string += '<div id="'+question_id+'-specify" class="response inline hidden">';
    ...
    var submit = '<div>'+
                  '<br><button type="button" id="submit" style="margin-bottom:15px">'+trial.submit+'</button>'+
                  '</div>';
...
***/
    ...
    $('.multiple.answer').on('click', function(e){
      ...
      // mark the relevant button as selected
      $('.multiple.answer[name='+name+']').each(function(i,d){
        ...
        if(match){
          ...
          // If this is a landing page with a language choice, update the continue button text with the chosen language
          if(trial.button_change){
            updateButton(option_value);
          }
        } else {
          current_option.removeClass('selected');
        }
      });
      // if this was highlighted in red (as an unanswered question), remove the highlighting
      $('#question-container-'+name).removeClass('problem');
      // check if this question has any followups and if so and if right choice made, reveal followups
      display_logic[name].forEach(function(d){
        if(d.type=='followup'){
          var reveal = false;
          if(Array.isArray(d.criterion)){
            if(d.criterion.indexOf(option_value)!=-1){
              reveal = true;
            }
          } else {
    ...
    $('input[type=checkbox]').on('change', function(e){
        ...
        if(d.type=='specify'){
          if(value == d.unhide_on){
            if(clicked_box.checked){
              ...
              $("#"+name+'-specify').removeClass('specify').addClass('optional hidden');
              $("input[name="+name+'-specify]').removeClass('specify').addClass('optional');
            }
          }
        }
        if(d.type=='followup'){
      ...
      // handle still other display logic (e.g. disabling on 'none' etc.);
      if(disable[value]){
        if(disable[value].indexOf(name)!=-1){
          $('#'+name+' input').each(function(i, d){
            if(d.value != value){
              if(clicked_box.checked){
                $(d).addClass('disabled');
...
****/
    ...
    function updateButton(value){
      $('#submit').html(trial.button_change[value])
    }
